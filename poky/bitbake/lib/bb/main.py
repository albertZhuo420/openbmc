...
def bitbake_main(configParams, configuration):
	...
	##
	# server_only 和 remote_server 是来自命令行设定的参数;
	# #
	...
	if configParams.remote_server:
		...
		while retries:
			try:
				...
				sockname = topdir + "/bitbake.sock" # /build/tiogapass/bitbake.sock
				if lock:
					...
					bb.event.ui_queue = []
					# wsl卡在  bb.server.process.BitBakeServer();
					# log: ERROR: Unable to start bitbake server (None)\
					# print("============" + str(sys._getframe().f_lineno))
					# print("============" + str(lock)) # <_io.TextIOWrapper name='/mnt/d/code/source_code_分析/openbmc_src/build/tiogapass/bitbake.lock' mode='a+' encoding='UTF-8'>
					# print("============" + str(sockname))
					# print("============" + str(featureset)) # [2, 1]
					# print("============" + str(configParams.server_timeout))	# none
					# print("============" + str(configParams.xmlrpcinterface))	# (None, 0)
					server = bb.server.process.BitBakeServer(lock, sockname, featureset, 
															 configParams.server_timeout, 
															 configParams.xmlrpcinterface)
				else:
